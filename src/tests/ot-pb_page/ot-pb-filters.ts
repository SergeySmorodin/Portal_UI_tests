...
//
// Протокол есть только у «Промышленной безопасности» и «Охраны труда» (у
// «Охраны труда» фильтрация по нему сейчас сломана на фронтенде — тест падает
// и фиксирует баг), удостоверение — только у «Охраны труда» и «Медицинской
// комиссии», поэтому поля протокола/удостоверения опциональны: если колонки
// на странице нет, соответствующий тест пропускается.
const pageMeta: Record<OtPbPageKey, OtPbPageMeta> = {
  industrialSafety: { startColumn: 4, stopColumn: 5, protocolColumn: 3 },
  laborProtection: { startColumn: 5, stopColumn: 6, protocolColumn: 3, certificateColumn: 4 },
...
export function runOtPbTests(pageKey: OtPbPageKey, cfg: OtPbTestConfig): void {
  test.describe(cfg.name, () => {
    ...
    test('Фильтрация по протоколу', async ({
      ...
      // Поиск по протоколу реализован фронтендом только на страницах
      // «Промышленная безопасность» (там поле to_protocol фильтруемых данных —
      // строка) и «Охрана труда», где фильтрация по нему сейчас сломана: поле
      // to_protocol — объект, поэтому тест падает и фиксирует баг. На
      // «Медицинской комиссии» ключа to_protocol нет вовсе, поэтому там тест
      // пропускается.
      test.skip(
        protocolColumn === undefined,
        'Фильтр по протоколу поддерживается только на страницах «Промышленная безопасность» и «Охрана труда»' // fixme
      );
